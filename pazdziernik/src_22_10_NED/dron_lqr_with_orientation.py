...
if engines_on_start:
    initial_thrust = MASS * g / 2.0  # Nominal hover thrust
else:
...
if n == 6:
    ...
    x = np.array([0.0, 0.0, 0.0, 0.0, z_initial, 0.0, initial_thrust, initial_thrust])
else:
    raise ValueError("n must be 6 or 8")

# Control input - start with initial thrust setting
u = np.array([initial_thrust, initial_thrust])

# Print startup configuration
print("\n" + "="*70)
print("DRONE LQR SIMULATION - NED COORDINATE SYSTEM")
print("="*70)
print(f"Model: n = {n} ({'with thrust dynamics' if n==8 else 'simple model'})")
...
print(f"Nominal hover thrust: {MASS * g / 2.0:.2f} N")
...
ax1.set_title('Drone Flight Simulation - NED Coordinates')
...
for i in range(10000):
    ...
    thrust_scale = 0.05  # Scale factor for visualization (zmniejszone bo dron większy)
