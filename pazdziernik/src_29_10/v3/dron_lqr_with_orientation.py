...
ax1.set_xlabel('X [m]')
ax1.set_ylabel('Z [m]')
ax1.set_title('Trajektoria Z(X)', fontsize=10)
...
# Offset for boundary of motor arm


x_boundary_minus = np.floor(-MOTOR_ARM_LENGTH_X)
x_boundary_plus = np.ceil(MOTOR_ARM_LENGTH_X)

y_boundary_minus = np.floor(-MOTOR_ARM_LENGTH_Y)
y_boundary_plus = np.ceil(MOTOR_ARM_LENGTH_Y)

print(f"X boundary: [{x_boundary_minus}, {x_boundary_plus}]")
print(f"Y boundary: [{y_boundary_minus}, {y_boundary_plus}]")

# Subplot 4: Drone Orientation (bottom right) - 3D VIEW
ax4 = plt.subplot(2, 2, 4, projection='3d')
ax4.set_xlim(x_boundary_minus, x_boundary_plus)
ax4.set_ylim(y_boundary_minus, y_boundary_plus)
ax4.set_zlim(y_boundary_minus, y_boundary_plus)

ax4.set_xticks(np.arange(x_boundary_minus, x_boundary_plus, 0.5))
ax4.set_yticks(np.arange(y_boundary_minus, y_boundary_plus, 0.5))
ax4.set_zticks(np.arange(y_boundary_minus, y_boundary_plus, 0.5))
...
for i in range(10000):
    ...
    xs, zs = aa_mdl(X_pos, Z_pos, theta_vis, 5)
    xsy, ys = aa_mdl(X_pos, Y_pos, x[9], 5)
    ...
    # Dynamiczny kolor tła w zależności od przekroczenia limitów
    ...
    ax1.text(0.98, 0.98, txt,
             ...
             family='monospace',  # Czcionka monospace dla wyrównania liczb
    ...
    # Plot drone model
    ax2.plot(xsy[:5], ys[:5], 'k', linewidth=3)
    ax2.set_xlabel('X [m]')
    ax2.set_ylabel('Y [m]')
    ax2.set_title('Trajektoria Y(X)', fontsize=10)
    ...
    ax4.set_xticks(np.arange(x_boundary_minus, x_boundary_plus, 0.5))
    ax4.set_yticks(np.arange(y_boundary_minus, y_boundary_plus, 0.5))
    ax4.set_zticks(np.arange(x_boundary_minus, x_boundary_plus, 0.5))

    ax4.view_init(elev=20, azim=45)

    # Draw world coordinate axes
    axis_len = x_boundary_plus + 0.5
    ax4.plot([0, 0], [0, axis_len], [0, 0], color='black', linewidth=3, alpha=0.7, label='X (world)')
    ax4.plot([0, axis_len], [0, 0], [0, 0], color='black', linewidth=3, alpha=0.7, label='Y (world)')
    ax4.plot([0, 0], [0, 0], [0, axis_len], color='black', linewidth=3, alpha=0.7, label='Z (world)')

    ax4.text(0, axis_len, 0, 'X', fontsize=14, color='black', weight='bold')
    ax4.text(axis_len, 0, 0, 'Y', fontsize=14, color='black', weight='bold')
    ax4.text(0, 0, axis_len, 'Z', fontsize=14, color='black', weight='bold')
    ...
    axis_body_len = MOTOR_ARM_LENGTH_X * 1.8
    ...
    if abs(theta_deg) > 1:
        draw_angle_arc_3d(ax4, np.array([0, 0, 0]), angle_radius,
