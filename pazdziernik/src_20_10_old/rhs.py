"""
RIGHT-HAND SIDE - POPRAWIONY MODEL FIZYCZNY
Równania ruchu drona w układzie CIAŁA (body frame) z wyrazami Coriolisa

KLUCZOWE ZMIANY:
- Prędkości vx, vz są w układzie CIAŁA (nie inercjalnym!)
- Dodane wyrazy Coriolisa: -omega*vz i +omega*vx
- Transformacja prędkości do układu inercjalnego dla pozycji
- Siły działają w układzie ciała, potem transformowane
- Zgodne z oryginalnym MATLAB
"""
...
def aa_rhs(x, t, u, az_turbulence=0.0, ax_wind=0.0, az_wind=0.0):
    ...
    POPRAWIONY MODEL - układ ciała (body frame)
    ...
    # Kąt natarcia w układzie ciała
    alpha = np.arctan2(vz_wind, vx_wind)
    ...
    L = 0.0             # Siła nośna (Lift) - brak skrzydeł
    ...
    # Kąt nachylenia silników (beta = 0 → silniki pionowo)
    beta = 0.0  # rad
    cb = np.cos(beta)
    sb = np.sin(beta)
    ...
    az = ((-D * np.sin(alpha) - L * np.cos(alpha) + G * np.cos(theta) - Thrust_1 * cb - Thrust_2 * cb) / MASS +
          omega * vx_wind + 
          az_turbulence)
    ...
    Mt = MOTOR_ARM_LENGTH * (Thrust_2 * cb - Thrust_1 * cb)
...
def diagnostyka_sil(x, u, az_turbulence=0.0):
    ...
    print("=" * 70)
    print(f"Prędkość V = {V:.3f} m/s, kąt natarcia α = {np.degrees(alpha):.2f}°")
    print(f"Kąt pochylenia θ = {np.degrees(theta):.2f}°")
    print()
    
    print("SIŁY W OSI X CIAŁA:")
    print(f"  Opór:         {-D * np.cos(alpha):+8.2f} N")
    print(f"  Ciężar:       {-G * np.sin(theta):+8.2f} N")
    print(f"  Suma:         {-D * np.cos(alpha) - G * np.sin(theta):+8.2f} N")
    print(f"  Przyspieszenie: {(-D * np.cos(alpha) - G * np.sin(theta))/MASS:+8.3f} m/s²")
    print()
    
    print("SIŁY W OSI Z CIAŁA:")
    print(f"  Opór:         {-D * np.sin(alpha):+8.2f} N")
    print(f"  Ciężar:       {+G * np.cos(theta):+8.2f} N")
    print(f"  Ciąg T1:      {-T1:+8.2f} N")
    print(f"  Ciąg T2:      {-T2:+8.2f} N")
    print(f"  Turbulencja:  {az_turbulence:+8.2f} m/s²")
    print(f"  Suma sił:     {-D * np.sin(alpha) + G * np.cos(theta) - T1 - T2:+8.2f} N")
    print(f"  Przyspieszenie: {(-D * np.sin(alpha) + G * np.cos(theta) - T1 - T2)/MASS + az_turbulence:+8.3f} m/s²")
    print()
    
    print("MOMENTY:")
    print(f"  Moment od ciągów: {MOTOR_ARM_LENGTH * (T2 - T1):+8.2f} N·m")
    print(f"  Moment tłumienia: {CM_Q * omega:+8.2f} N·m")
    print(f"  Przyspieszenie kątowe: {(MOTOR_ARM_LENGTH * (T2 - T1) + CM_Q * omega) / IY:+8.3f} rad/s²")
    print("=" * 70)
    print()
