...
def _generate_synthetic(Vel, dt, avoid_distance):
    """Generuje pomocniczy profil (backup gdy brak planned_path.npz)"""
...
def generate_reference_profile(Vel, dt, avoid_distance):
    ...
    if PLANNED_PATH.exists():
        try:
            ...
            print(f"[trajectory] Błąd wczytywania planned_path.npz: {e}")
            print("[trajectory] Używam profilu pomocniczego.")
    else:
        print(f"[trajectory] Brak pliku {PLANNED_PATH}. Uzywam profilu pomocniczego.")
