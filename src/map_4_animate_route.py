...
class DroneLiveAnimator:
    ...
    def initialize(self, path_result, fig=None, gridspec=None):
        ...
        self.ax1.set_title('Profil trasy (X, Y, Z)')
        ...
        self.ax2.set_title("Widok z góry")
        ...
        self.ax2.set_xlim(
            min(min_temp, xa, xb) - margin_xy,
            max(max_temp, xa, xb) + margin_xy
        )

        self.ax2.set_ylim(
            min(min_temp, ya, yb) - margin_xy,
            max(max_temp, ya, yb) + margin_xy
        )
