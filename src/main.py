...
def section_instruction():
    ...
    print()
    print("PYTANIA I ODPOWIEDZI".center(80))
    print("Prawidłowe odpowiedzi do wszystkich pytań w programie:")
    print("  - pozytywne: 't', 'tak', 'y', 'yes'")
    print("  - negatywne: 'n', 'nie', 'no'")
    print("Wciśnij 'Enter', aby użyć domyślnej konfiguracji podanej w nawiasach kwadratowych [].")

    print()
    print("WYKRESY".center(80))
    print("Po wybraniu miasta oraz trybu interaktywnego należy wybrać dwa punkty na mapie.")
    print("Aby to zrobić, naciśnij klawisz SPACJA, a następnie kursorem zaznacz na mapie dwa punkty.")
    print()
    print("Widok mapy można zmieniać za pomocą paska narzędzi w lewym dolnym rogu okna wykresu.")
    print("Opis przycisków:")
    print("  Domek        - powrót do domyślnego widoku")
    print("  Strzałka ←   - cofnięcie ostatniej zmiany widoku")
    print("  Strzałka →   - ponowienie (krok do przodu) zmiany widoku")
    print("  Róża wiatrów - przesuwanie (panowanie) widoku")
    print("  Lupa         - przybliżanie/oddalanie (zoom)")
    print("  Suwaki       - konfiguracja widoku wykresu")
    print("  Dyskietka    - zapisanie wykresu do pliku (np. PNG)")
    print()
    ...
    while True:
        user_input = input("Czy zapoznano się z instrukcją użytkowania? [t]: ").strip().lower()

        if user_input in ['t', 'tak', 'y', 'yes', '']:
            # potwierdzono
            break   # kończymy pętlę i idziemy dalej
        elif user_input in ['n', 'nie', 'no']:
            print("Proszę zapoznać się z powyższą instrukcją użytkownika.")
            print("Gdy będziesz gotowy/a, odpowiedz ponownie.")
        else:
            print("Błędna odpowiedź. Dozwolone odpowiedzi: t/tak/y/yes lub n/nie/no.")
...
def section_2_route_planning(G, buildings, city_id):
    ...
    def handle_buildings(point_a, point_b):
        ...
        if h_start is not None:
            ...
            ans = input("Czy chcesz WYSTARTOWAĆ z dachu budynku? [t]: ").strip().lower()
        ...
        if h_end is not None:
            ...
            ans = input("Czy chcesz WYLĄDOWAĆ na dachu budynku? [t]: ").strip().lower()
            if ans in ["", "t", "tak", "y", "yes"]:
                roof_end = h_end
            else:
                print("OK, wracamy do wyboru punktów A i B.")
...
def main():
    ...
    print_header("SYSTEM SYMULACJI DRONA")
    print("1. Wybór i pobieranie/ładowanie danych miasta")
    print("2. Wybór punktów A i B na mapie")
    print("3. Konfiguracja parametrów drona")
    print("4. Planowanie ścieżki z omijaniem budynków")
    print("5. Symulacja lotu z wizualizacją")

    print_header("INSTRUKCJA UŻYTKOWNIKA")
    section_instruction()

    print_header("SEKCJA 1: WYBÓR MIASTA")
