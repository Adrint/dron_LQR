...
#global variables
from constants import (z0, Vel, X_turb_1, X_turb_2, c_turb)
...
for i in range(10000):
    ...
    tp.append(t)  # czas w danej iteracji
    yp.append(x.copy())  # wektor stanu drona (np. prędkości, pozycje, kąt, itp.)
    up.append(u.copy())  # sterowanie (ciągi T1, T2)
    ...
    omega_ref = 0.0
    theta_ref = 0.0
    # Oblicz parametry ciągu
    T1 = u[0]
    T2 = u[1]
    T = T1 + T2  # sumaryczna siła ciągu
    theta = x[5]
    Fx = T * np.sin(theta)  # składowa pozioma
    Fz = T * np.cos(theta)  # składowa pionowa

    #if i < 200:
    print("========== DIAGNOSTYKA ==========")
    print(f"{'Czas':<10} | {t:9.2f}sek |")
    print(f"{'Zmienna':<10} | {'Referencja':>12} | {'Aktualna':>12} | {'Blad':>12} | Opis")
    print("-" * 80)

    # vx – prędkość wzdłuż osi drona (do przodu)
    print(f"{'vx':<10} | {vx_ref:12.5f} | {x[0]:12.5f} | {e[0]:12.5f} | "
          f"prędkość wzdłuż kadłuba (dodatnia - do przodu)")

    # vz – prędkość w pionie (dodatnia = do góry)
    print(f"{'vz':<10} | {-vz_ref:12.5f} | {-x[1]:12.5f} | {-e[1]:12.5f} | "
          f"wznoszenie/opadanie (dodatnia - w górę)")

    # omega – prędkość kątowa (skręcanie)
    print(f"{'omega':<10} | {omega_ref:12.5f} | {x[2]:12.5f} | {e[2]:12.5f} | "
          f"prędkość obrotu wokół środka (rad/s)")

    # X – pozycja w poziomie
    print(f"{'X':<10} | {x_ref:12.5f} | {x[3]:12.5f} | {e[3]:12.5f} | "
          f"pozycja pozioma wzdłuż trajektorii")

    # Z – wysokość (dodatnia = wyżej nad ziemią)
    print(f"{'Z':<10} | {z_ref:12.5f} | {-x[4]:12.5f} | {-e[4]:12.5f} | "
          f"wysokość nad terenem (dodatnia)")

    # theta – kąt nachylenia (pitch)
    print(f"{'theta':<10} | {math.degrees(theta_ref):12.5f} | {math.degrees(x[5]):12.5f} | {math.degrees(e[5]):12.5f} | "
    ...
    if i % 20 == 0:
        ...
        # ❌ NIE ustawiamy plt.axis(...) tutaj ponownie
        # ✅ Zakładamy, że zostało ustawione na początku:
        # plt.axis([0, 50.0, 0.0, 20.0])
