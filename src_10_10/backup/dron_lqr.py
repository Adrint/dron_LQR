...
# --- Stałe referencje:
X_ref_all, Z_terr_all, Z_ref_all, alpha_all = generate_reference_profile(Vel, dt)
...
for i in range(10000):
    ...
    # === 2. Zaktualizuj pozycję referencyjną na podstawie kąta nachylenia ===
    x_ref += Vel * np.cos(alfa) * dt

    # === 3. Składowe prędkości w kierunku trajektorii ===
    Vx = Vel * np.cos(alfa)
    Vz = Vel * np.sin(alfa)
