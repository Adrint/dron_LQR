...
def section_4_path_planning(point_a, point_b, buildings, city_id,
                            roof_start_alt=None, roof_end_alt=None):
    # Nadpisujemy wysokości startu/lądowania, jeśli użytkownik wybrał dachy
    if roof_start_alt is not None:
        print(f"\nUwaga: punkt A na dachu – ustawiam wysokość startową na {roof_start_alt:.1f} m.")
        config.altitude_start = roof_start_alt
    else:
        # brak dachu -> NIE pozwalamy startować wewnątrz budynku
        config.allow_start_inside_building = False

    if roof_end_alt is not None:
        print(f"Uwaga: punkt B na dachu – ustawiam wysokość lądowania na {roof_end_alt:.1f} m.")
        config.altitude_end = roof_end_alt
    else:
        # brak dachu -> NIE pozwalamy lądować wewnątrz budynku
        config.allow_end_inside_building = False
    ...
    path_result = planner.plan(point_a, point_b, roof_start_alt, roof_end_alt)
    return path_result
...
def main():
    ...
    path_result = section_4_path_planning(point_a, point_b, buildings, city_id,
                                          roof_start_alt, roof_end_alt)
    # *** NOWE – jeśli nie ma ścieżki, NIE uruchamiamy LQR ***
