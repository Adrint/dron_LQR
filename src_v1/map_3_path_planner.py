...
class DroneRoutePlanner:
    ...
    def _add_vertical_segments(self, smoothed_path, start_alt, cruise_min, cruise_max, end_alt,
        ...
        if abs(start_alt - cruise_min) > 1e-3:
            num_steps = max(int(abs(cruise_min - start_alt) / resolution) + 1, 3)
            for i in range(num_steps + 1):
                t = i / num_steps
                # liniowo od start_alt -> cruise_min (w górę lub w dół)
                z = start_alt + (cruise_min - start_alt) * t
        ...
        if abs(end_alt - cruise_min) > 1e-3:
            num_steps = max(int(abs(cruise_min - end_alt) / resolution) + 1, 3)
            for i in range(num_steps + 1):
                t = i / num_steps
                # liniowo od cruise_min -> end_alt
                z = cruise_min + (end_alt - cruise_min) * t
        ...
        # ======================
        full_path = smoothed_path.copy()

        if takeoff_waypoints:
            full_path = takeoff_waypoints + full_path
        if landing_waypoints:
            full_path = full_path + landing_waypoints
    ...
    def plan(self, point_a, point_b, roof_start_alt=None, roof_end_alt=None):
        ...
        # Nadpisanie pierwszego punktu Z, jeśli startujemy z dachu
        if roof_start_alt is not None and len(Z) > 0:
            Z[0] = -roof_start_alt  # NED: ujemne = wyżej
