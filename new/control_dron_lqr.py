...
def dron_lqr(path_result=None):
    ...
    for i in range(max_iterations):
        ...
        # 6. Obliczanie błędów do punktu docelowego (dla monitorowania)
        x_goal = x_ref_all[-1]
        y_goal = y_ref_all[-1]
        z_goal = z_ref_all[-1]
        ...
        # PROGI ZAKOŃCZENIA - dokładne lądowanie
        HORIZ_TOL = 5.0  # [m] - margines poziomy ±5m
        VERT_TOL = 1.0  # [m] - margines pionowy ±1m (dokładne lądowanie)
        VEL_HORIZ_TOL = 1.0  # [m/s] - próg prędkości poziomej
        VEL_VERT_TOL = 0.5  # [m/s] - próg prędkości pionowej (opadanie)

        # 7. Wyświetlanie statusu
        if i % print_interval == 0 and i > 0:
            # Sprawdź czy dron jest blisko celu poziomo ale nie pionowo
            if horiz_err < HORIZ_TOL * 2 and vert_err > VERT_TOL:
                print(f"t={t:6.2f}s | Pozycja: X={x[6]:7.2f} Y={x[7]:7.2f} Z={x[8]:7.2f} | "
                      f"AGL={-x[8]:6.2f}m | s={s_ref:7.2f}m | "
                      f"⚠ BLISKO CELU XY, OPADANIE... (blad Z: {vert_err:.1f}m)")
            else:
                print(f"t={t:6.2f}s | Pozycja: X={x[6]:7.2f} Y={x[7]:7.2f} Z={x[8]:7.2f} | "
                      f"AGL={-x[8]:6.2f}m | s={s_ref:7.2f}m")
        ...
        # ========================================================================
        # 8. WARUNEK KOŃCA - dokładne lądowanie
        # ========================================================================

        # Prędkość pozioma i pionowa
        vel_horiz = math.hypot(x[0], x[1])
        vel_vert = abs(x[2])

        # Warunek końca: dron musi być blisko celu I poruszać się powoli
        # KRYTYCZNE: Wszystkie warunki muszą być spełnione jednocześnie
        if (horiz_err < HORIZ_TOL and
                vert_err < VERT_TOL and
                vel_horiz < VEL_HORIZ_TOL and
                vel_vert < VEL_VERT_TOL):
            print(f"\n{'=' * 80}")
            print("OSIAGNIETO CEL - LADOWANIE ZAKONCZONE!")
            print(f"{'=' * 80}")
            print(f"Czas lotu: {t:.2f}s")
            print(f"\nDOKLADNOSC POZYCJI:")
            print(f"  Blad poziomy (XY): {horiz_err:.2f}m (limit: {HORIZ_TOL}m)")
            print(f"  Blad pionowy (Z):  {vert_err:.2f}m (limit: {VERT_TOL}m)")
            print(f"\nPREDKOSCI KONCOWE:")
            print(f"  Pozioma (XY): {vel_horiz:.2f}m/s (limit: {VEL_HORIZ_TOL}m/s)")
            print(f"  Pionowa (Z):  {vel_vert:.2f}m/s (limit: {VEL_VERT_TOL}m/s)")
            print(f"\nPOZYCJA KONCOWA:")
            print(f"  X = {x[6]:.2f}m")
            print(f"  Y = {x[7]:.2f}m")
            print(f"  Z = {x[8]:.2f}m (NED)")
            print(f"  Wysokosc AGL = {-x[8]:.2f}m")
            print(f"{'=' * 80}")

            # Wyłącz silniki po osiągnięciu celu
