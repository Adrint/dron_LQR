...
class DroneConfig:
    ...
    def set_params(self, **kwargs):
        ...
        # (zostały już obliczone w main.py w trybie auto lub podane przez użytkownika)
        ...
        I_engines_x = 4 * (self.mass_engine * self.motor_arm_y ** 2 + I_motor_own)

        # Pitch (obrót wokół Y): silniki odległe o motor_arm_x od osi Y
        I_engines_y = 4 * (self.mass_engine * self.motor_arm_x ** 2 + I_motor_own)

        # Yaw (obrót wokół Z): silniki odległe o sqrt(motor_arm_x² + motor_arm_y²) od osi Z
        I_engines_z = 4 * (self.mass_engine * (self.motor_arm_y ** 2 + self.motor_arm_x ** 2) + I_motor_own)
    ...
    def print_inertia_breakdown(self):
        """Wyświetl szczegółowy rozkład momentów bezwładności"""
        print("\n" + "=" * 80)
        ...
        print(f"   Ramiona: X={self.motor_arm_x:.3f}m, Y={self.motor_arm_y:.3f}m")
