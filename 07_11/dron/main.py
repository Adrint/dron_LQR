...
def drone_parameters():
    ...
    # Masa drona
    mass_input = input("Masa drona [kg] [20.0]: ").strip()
    mass = float(mass_input) if mass_input else 20.0

    # Masa silnika
    mass_engine_input = input("Masa pojedynczego rotora [kg] [2.0]: ").strip()
    mass_engine = float(mass_engine_input) if mass_engine_input else 2.0

    # Długość ramienia X
    motor_arm_x_input = input("Długość ramienia (od środka do rotoru) [m] [1]: ").strip()
    motor_arm_x = float(motor_arm_x_input) if motor_arm_x_input else 1
    motor_arm_y = motor_arm_x

    # WYMIARY KORPUSU DRONA (dla momentów bezwładności)
    # ========================================================================
    print("\n--- Wymiary korpusu drona ---")

    body_length_input = input("  Długość korpusu (oś X, forward) [m] [0.4]: ").strip()
    body_length = float(body_length_input) if body_length_input else 0.4

    body_width_input = input("  Szerokość korpusu (oś Y, lateral) [m] [0.4]: ").strip()
    body_width = float(body_width_input) if body_width_input else 0.4

    body_height_input = input("  Wysokość korpusu (oś Z, vertical) [m] [0.4]: ").strip()
    body_height = float(body_height_input) if body_height_input else 0.4
    ...
    return mass, mass_engine, motor_arm_x, motor_arm_y, body_length, body_width, body_height, CD_0, k_torque, tau, C_Lp, CM_Q, C_Nr
...
def limits():
    ...
    max_psi_angle_input = input("Maksymalny kąt  (psi) [°] [20]: ").strip()
    max_psi_angle_deg = float(max_psi_angle_input) if max_psi_angle_input else 20.0
...
def flight_parameters():
    ...
    velocity_input = input("Prędkość referencyjna drona [m/s] [6.0]: ").strip()
    velocity = float(velocity_input) if velocity_input else 6.0

    # Wysokości
    print("\n--- Wysokości lotu ---")
    h_start_input = input("Wysokość startowa [m] [2.0]: ").strip()
    h_start = float(h_start_input) if h_start_input else 3.0
...
def path_analise_parameters():
    ...
    # Krok czasowy
    dt_input = input("Krok czasowy dt [s] [0.01]: ").strip()
    ...
    return n, include_thrust_dynamics, m, t, dt
            #MAX_PATH_ITERATIONS, DETOUR_SEARCH_ANGLES, DETOUR_DISTANCE_MULTIPLIER, PATH_SMOOTHING_POINTS, COLLISION_CHECK_SAMPLES)

def main():
    print("Wciśnij Enter aby użyć wartości domyślnej\n")
    # Parametry drona
    mass, mass_engine, motor_arm_x, motor_arm_y, body_length, body_width, body_height, CD_0, k_torque, tau, C_Lp, CM_Q, C_Nr = drone_parameters()
    ...
    # Parametry analizy trajektorii
    n, include_thrust_dynamics, m, t, dt = path_analise_parameters()
