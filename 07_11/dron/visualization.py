...
from rhs import rotation_matrix_zyx
...
class DroneVisualizer:
    ...
    def __init__(self, X_ref_all, Y_terr_all, Z_terr_all, Y_ref_all, Z_ref_all):
        ...
        self.motor_arm_x = config.motor_arm_x
        self.motor_arm_y = config.motor_arm_y
        ...
        self.x_boundary_minus = np.floor(-self.motor_arm_x)
        self.x_boundary_plus = np.ceil(self.motor_arm_x)
        self.y_boundary_minus = np.floor(-self.motor_arm_y)
        self.y_boundary_plus = np.ceil(self.motor_arm_y)
    ...
    def _draw_drone_orientation(self, x, T1, T2, T3, T4, phi_deg, theta_deg, psi_deg):
        ...
        self.ax4.set_xlabel('X [m]', fontsize=11, weight='bold')
        self.ax4.set_ylabel('Y [m]', fontsize=11, weight='bold')
        ...
        #self.ax4.view_init(elev=20, azim=135)

        # Draw world coordinate axes
        axis_len = self.x_boundary_plus + 0.5
        self.ax4.plot([0, axis_len], [0, 0], [0, 0], color='black', linewidth=3, alpha=0.7, label='X (world)')
        self.ax4.plot([0, 0], [0, axis_len], [0, 0], color='black', linewidth=3, alpha=0.7, label='Y (world)')
        self.ax4.plot([0, 0], [0, 0], [0, axis_len], color='black', linewidth=3, alpha=0.7, label='Z (world)')

        self.ax4.text(axis_len, 0, 0, 'X', fontsize=14, color='black', weight='bold')
        self.ax4.text(0, axis_len, 0, 'Y', fontsize=14, color='black', weight='bold')
        ...
        # Drone dimensions
        arm_x = self.motor_arm_x
        arm_y = self.motor_arm_y

        # Motor positions in body frame (X configuration)
        angle_offset = np.pi / 4  # 45 degrees
        motors_body = [
            np.array([arm_x * np.cos(angle_offset), arm_y * np.sin(angle_offset), 0]),
            np.array([arm_x * np.cos(angle_offset), -arm_y * np.sin(angle_offset), 0]),
            np.array([-arm_x * np.cos(angle_offset), arm_y * np.sin(angle_offset), 0]),
            np.array([-arm_x * np.cos(angle_offset), -arm_y * np.sin(angle_offset), 0])
        ]
        ...
        self.ax4.plot([motors_world[0][0], motors_world[3][0]],
                      [motors_world[0][1], motors_world[3][1]],
                      [motors_world[0][2], motors_world[3][2]],
                      'k-', linewidth=3, alpha=0.9)
        self.ax4.plot([motors_world[1][0], motors_world[2][0]],
        ...
        for j, (m_pos, T) in enumerate(zip(motors_world, thrusts)):
            self.ax4.scatter([m_pos[0]], [m_pos[1]], [m_pos[2]],
        ...
        self.ax4.quiver(0, 0, 0, x_body_dir[0], x_body_dir[1], x_body_dir[2],
                        color='red', arrow_length_ratio=0.2, linewidth=3, linestyle='--', alpha=0.9)
        self.ax4.text(x_body_dir[0] * 1.1, x_body_dir[1] * 1.1, x_body_dir[2] * 1.1,
                      'X\'', fontsize=12, color='red', weight='bold')

        # Y_body (right) - GREEN
        y_body_dir = rotate_point([0, axis_body_len, 0], R)
        self.ax4.quiver(0, 0, 0, y_body_dir[0], y_body_dir[1], y_body_dir[2],
                        color='green', arrow_length_ratio=0.2, linewidth=3, linestyle='--', alpha=0.9)
        self.ax4.text(y_body_dir[0] * 1.1, y_body_dir[1] * 1.1, y_body_dir[2] * 1.1,
                      'Y\'', fontsize=12, color='green', weight='bold')

        # Z_body (down) - BLUE
        z_body_dir = rotate_point([0, 0, axis_body_len], R)
        self.ax4.quiver(0, 0, 0, z_body_dir[0], z_body_dir[1], z_body_dir[2],
                        color='blue', arrow_length_ratio=0.2, linewidth=3, linestyle='--', alpha=0.9)
        self.ax4.text(z_body_dir[0] * 1.1, z_body_dir[1] * 1.1, z_body_dir[2] * 1.1,
