...
def limit_angular_velocities(p, q, r, phi, theta, psi):
    """
    Intelligent angular velocity limiting with soft saturation
    ...
    # Instead of hard zeroing, apply scaling factor that smoothly reduces rate
    margin = 0.087  # ~5 degrees margin for soft transition

    # Theta (pitch) - soft saturation
    if theta > max_theta_angle_rad - margin and q_limited > 0:
        # Approaching upper limit while pitching up
        scale = max(0.0, (max_theta_angle_rad - theta) / margin)
        q_limited = scale * q_limited
    elif theta < -max_theta_angle_rad + margin and q_limited < 0:
        # Approaching lower limit while pitching down
        scale = max(0.0, (max_theta_angle_rad + theta) / margin)
        q_limited = scale * q_limited

    # Phi (roll) - soft saturation
    if phi > max_phi_angle_rad - margin and p_limited > 0:
        scale = max(0.0, (max_phi_angle_rad - phi) / margin)
        p_limited = scale * p_limited
    elif phi < -max_phi_angle_rad + margin and p_limited < 0:
        scale = max(0.0, (max_phi_angle_rad + phi) / margin)
        p_limited = scale * p_limited

    # Psi (yaw) - soft saturation
    if psi > max_psi_angle_rad - margin and r_limited > 0:
        scale = max(0.0, (max_psi_angle_rad - psi) / margin)
        r_limited = scale * r_limited
    elif psi < -max_psi_angle_rad + margin and r_limited < 0:
        scale = max(0.0, (max_psi_angle_rad + psi) / margin)
        r_limited = scale * r_limited
...
def limit_angles(phi, theta, psi):
    """
    Limit angles with hard saturation
...
def apply_all_limits(x):
    ...
    p_lim, q_lim, r_lim = limit_angular_velocities(p, q, r, phi, theta, psi)
...
def check_limits_exceeded(x, verbose=False):
    ...
    if abs(theta) > max_theta_angle_rad:
        violations.append(
            f"Pitch angle exceeded: {np.rad2deg(abs(theta)):.1f}° > {np.rad2deg(max_theta_angle_rad):.1f}°")
    ...
    max_descent_used = max_descent_tilted if is_tilted else max_descent

    if vz > max_descent_used:
        violations.append(f"Descent rate exceeded: {vz:.1f} m/s > {max_descent_used:.1f} m/s")
