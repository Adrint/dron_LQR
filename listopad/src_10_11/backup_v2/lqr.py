...
def lqr_m(a, b, q, r, nn=None):
    ...
    if max_real_part >= 0:
        warnings.warn(
            f"⚠️ UNSTABLE CLOSED-LOOP! Maximum eigenvalue real part: {max_real_part:.4f}\n"
            f"System may be unstable or marginally stable. Check Q and R matrices.",
            RuntimeWarning
        )
    elif max_real_part > -0.1:
        warnings.warn(
            f"⚠️ MARGINALLY STABLE! Maximum eigenvalue real part: {max_real_part:.4f}\n"
    ...
    if damping_ratios:
        ...
        if min_damping < 0.1:
            warnings.warn(
                f"⚠️ LOW DAMPING! Minimum damping ratio: {min_damping:.3f}\n"
