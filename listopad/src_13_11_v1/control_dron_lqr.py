...
def dron_lqr():
    ...
    s_ref_all, y_terr_all, z_terr_all, y_ref_all, z_ref_all, alpha_all, beta_all, x_ref_all = generate_reference_profile(
        velocity, dt, avoid_distance)
    visualizer = DroneVisualizer(s_ref_all, x_ref_all, y_terr_all, z_terr_all, y_ref_all, z_ref_all)
    ...
    if n == 12:
        ...
        x = np.array([0.0, 0.0, 0.0,  # vx, vy, vz
                      0.0, 0.0, 0.0,  # p, q, r
                      x_ref_all[0], y_ref_all[0], z_ref_all[0],  # x, y, z
    ...
    # Wczytaj punkty A i B z pliku (jeśli istnieje)
    import json
    from pathlib import Path

    points_file = Path("selected_points.json")
    if points_file.exists():
        try:
            with open(points_file, "r", encoding="utf-8") as f:
                points_data = json.load(f)
                point_a = (points_data["A"]["lat"], points_data["A"]["lon"])
                point_b = (points_data["B"]["lat"], points_data["B"]["lon"])
                print(f"\n✓ Wczytano punkty z {points_file}")
                print(f"  A: {point_a}")
                print(f"  B: {point_b}")
        except Exception as e:
            print(f"⚠️ Błąd wczytywania punktów: {e}")
            print("   Używam domyślnych punktów")
            point_a = None
            point_b = None
    else:
        print(f"⚠️ Brak pliku {points_file}, używam domyślnych punktów")
        point_a = None
        point_b = None

    planner = DroneRoutePlanner(config)
    res = planner.plan(point_a=point_a, point_b=point_b, show_plots=False)
    path_latlon = res["path_latlon"]

    # Użyj przefiltrowanych budynków (tylko z obszaru trasy)
    buildings_gdf = planner.buildings_in_area if hasattr(planner, 'buildings_in_area') else planner.buildings_gdf
    ...
    anim = PlannerStyleAnimator(
        ...
        sim_ref_s=s_ref_all  # <= KLUCZOWE: dystans referencyjny z symulacji
    )
    ...
    s_ref = 0.0  # Dystans przebity (zamiast x_ref)
    x_ref = 0.0  # Współrzędna X dla regulatora
    y_ref = 0.0
    z_ref = 0.0
    ...
    for i in range(1000):
        ...
        # ====================================================================
        # 1. Punkt referencyjny (śledź dystans zamiast X)
        # ====================================================================
        ...
        # ====================================================================
        # 6. Efekty środowiskowe (turbulencja na bazie dystansu)
        # ====================================================================
