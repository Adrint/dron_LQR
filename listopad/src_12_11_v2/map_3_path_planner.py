...
class DroneRoutePlanner:
    ...
    # === NEW: znajdź najbliższy wolny poziom k nad komórką (i,j) ===
    def _nearest_free_k(self, i, j, k_target, k_window=50):
        nx, ny, nz = self.grid_shape
        k_target = int(np.clip(k_target, 0, nz - 1))
        if not self.occupancy_grid[i, j, k_target]:
            return k_target
        for dk in range(1, k_window + 1):
            k1 = k_target - dk
            k2 = k_target + dk
            if 0 <= k1 < nz and not self.occupancy_grid[i, j, k1]:
                return k1
            if 0 <= k2 < nz and not self.occupancy_grid[i, j, k2]:
                return k2
        # jeśli brak wolnego w kolumnie, zwróć target (później zadziała _nearest_free)
        return k_target

    # === NEW: sąsiady z opcją „zamrożenia” wysokości (lock_k) dla etapu przelotowego ===
    def _neighbors_locked(self, node: Node, lock_k: Optional[int] = None):
        i, j, k = node.position
        for di in (-1, 0, 1):
            for dj in (-1, 0, 1):
                for dk in (-1, 0, 1):
                    ...
                    if lock_k is not None and kk != lock_k:
                        continue
                    if self._is_valid_node((ii, jj, kk)):
                        yield Node((ii, jj, kk), node)
    ...
    def _astar(self, start_pos, goal_pos, lock_k: Optional[int] = None):
        open_heap = []
        ...
        while open_heap and iterations < max_iter:
            ...
            for nb in self._neighbors_locked(current, lock_k=lock_k):
    ...
    def _plot_overview_and_profile(self, path_latlon, latA, lonA, latB, lonB, buildings):
        # --- Przygotowanie referencji/metrów + obrót jak w map_2 ---
        ...
        # --- Dystans wzdłuż trasy s (profil) ---
        dx = np.diff(px); dy = np.diff(py)
        ds = np.hypot(dx, dy)
        s = np.concatenate([[0.0], np.cumsum(ds)])
        alts_arr = np.array(alts, dtype=float)

        # --- Wysokość zabudowy pod trasą (jak dotychczas) ---
        ...
        # --- Linia prosta A–B + trasa planera ---
        ax1.plot([xa_r, xb_r], [ya_r, yb_r], "b-", lw=3, alpha=0.7, label="Linia prosta")
        ax1.plot(px_r, py_r, "r-", lw=3, label="Trasa drona", zorder=50)
        ax1.set_xlim(x_min, x_max)
        ax1.set_ylim(y_min, y_max)
        ax1.set_ylabel("Y [m]")
        ax1.set_title("Widok z góry Y(X) – zaplanowana trasa")
        ax1.grid(True, alpha=0.3, linestyle="--")
        ax1.legend(loc="upper right")

        # --- Wysokość zabudowy vs profil lotu ---
        ax2.fill_between(
            s, 0.0, bldg_h,
            facecolor=COLORS["building"],
            alpha=0.6,
            label="Wysokość zabudowy",
            zorder=20
        )
        ax2.plot(s, bldg_h, linewidth=0.5, color="darkgray", zorder=21)
        ax2.plot(s, alts_arr, "r-", lw=3, label="Wysokość trasy", zorder=100)

        ax2.set_xlabel("Dystans wzdłuż trasy s [m]")
        ax2.set_ylabel("Wysokość [m]")
        ax2.set_title("Wysokość zabudowy na zaplanowanej trasie")
        ax2.grid(True, alpha=0.3, linestyle="--")
        ax2.set_xlim(0, float(s[-1] if s.size else 1.0))
        ax2.set_ylim(
            bottom=0,
            top=max(
                float(alts_arr.max()) if alts_arr.size else 0.0,
                float(bldg_h.max()) if bldg_h.size else 0.0
            ) * 1.15 + 1e-6
        )
    ...
    def plan(self, point_a: Optional[Tuple[float, float]],
        ...
        # --- WYBÓR PUŁAPU PRZELOTOWEGO ---
        # preferuj środek korytarza, ale dostosuj do wolnej komórki nad startem/celem
        cruise_pref = 0.5 * (self.altitude_min + self.altitude_max)
        _, _, cruise_pref_k_world = self._world_to_grid(0.0, 0.0, cruise_pref)
        cruise_k_start = self._nearest_free_k(start_g[0], start_g[1], cruise_pref_k_world)
        cruise_k_goal  = self._nearest_free_k(goal_g[0],  goal_g[1],  cruise_pref_k_world)

        cruise_k = cruise_k_start if abs(cruise_k_start - cruise_pref_k_world) <= abs(cruise_k_goal - cruise_pref_k_world) else cruise_k_goal

        # etapy pośrednie na wspólnym k przelotowym
        start_cruise = (start_g[0], start_g[1], cruise_k)
        goal_cruise  = (goal_g[0],  goal_g[1],  cruise_k)

        # === ETAP 1: pionowy wznos/opad do pułapu przelotowego nad A
        leg1 = self._astar(start_g, start_cruise, lock_k=None)
        if leg1 is None:
            print("✗ Etap 1 (do przelotu) nieudany")
            return None

        # === ETAP 2: przelot A->B na stałej wysokości (lock_k)
        leg2 = self._astar(start_cruise, goal_cruise, lock_k=cruise_k)
        if leg2 is None:
            print("✗ Etap 2 (przelot) nieudany — zwiększ altitude_max lub zmień grid_resolution")
            return None

        # === ETAP 3: pionowe zejście/wznos do wysokości końcowej nad B
        leg3 = self._astar(goal_cruise, goal_g, lock_k=None)
        if leg3 is None:
            print("✗ Etap 3 (zejście do celu) nieudany")
            return None

        # sklej ścieżkę (usuń zdublowane węzły na łączeniach)
        path_grid = leg1[:-1] + leg2[:-1] + leg3
...
# --- DODATKOWE NARZĘDZIA (opcjonalne, jeśli używasz w innych modułach) ---
def compute_building_profile_along_path(x_m, y_m, buildings_gdf, ref_lat, ref_lon, angle_rad, latlon_to_meters, estimate_height):
    """
    x_m, y_m: próbki trasy w metrach (po obrocie jak na mapie)
    buildings_gdf: ten sam GeoDataFrame z budynkami, którego używasz do rysowania
    ref_lat, ref_lon: referencja projekcji
    angle_rad: kąt obrotu użyty do rysowania
    latlon_to_meters: Twoja funkcja (lat,lon)->(x,y,z) względem (ref_lat, ref_lon)
    estimate_height: Twoja funkcja wyznaczająca wysokość budynku z atrybutów
    Zwraca: 1D np.array bldg_h w metrach (maks. wysokość budynku „pod” próbką ścieżki)
    """
    import numpy as np
    from shapely.geometry import Point

    bldg_h = np.zeros_like(x_m, dtype=float)
    tol_deg = 1.5 / 111000.0  # ~1.5 m

    # odwróć obrót i projekcję, żeby sprawdzać w GEO
    cos_a, sin_a = np.cos(-angle_rad), np.sin(-angle_rad)

    for i, (xx, yy) in enumerate(zip(x_m, y_m)):
        # cofnięcie obrotu
        x0 =  xx * cos_a - yy * sin_a
        y0 =  xx * sin_a + yy * cos_a
        # (tu ewentualnie dodaj projekcję odwrotną x0,y0 -> lat,lon)
        if buildings_gdf is None or len(buildings_gdf) == 0:
            bldg_h[i] = 0.0
            continue
        around = buildings_gdf  # możesz zawęzić .cx jeśli masz dokładne (lat,lon)
        hmax = 0.0
        p = Point(0, 0)  # placeholder jeżeli nie masz tu dokładnych geo
        for _, b in around.iterrows():
            try:
                geom = b.geometry
                if geom is None:
                    continue
                if geom.contains(p):
                    hmax = max(hmax, estimate_height(b))
            except Exception:
                continue
        bldg_h[i] = hmax
...
def export_buildings_xy(buildings_gdf, ref_lat, ref_lon, angle_rad, latlon_to_meters):
    polys = []
    if buildings_gdf is None or len(buildings_gdf) == 0:
        return polys
    for _, b in buildings_gdf.iterrows():
        ...
        for g in geoms:
            ...
            for lonv, latv in coords:
                x, y, _ = latlon_to_meters(latv, lonv, 0, ref_lat, ref_lon)
                bx.append(x); by.append(y)
            bx = np.asarray(bx); by = np.asarray(by)
            cos_a, sin_a = np.cos(angle_rad), np.sin(angle_rad)
            bxr =  bx * cos_a + by * sin_a
            byr = -bx * sin_a + by * cos_a
            polys.append(np.column_stack([bxr, byr]))
    return polys
