...
class _Projector:
    ...
    def ll_to_m(self, lat, lon):
        x,y = self.to_m.transform(lon, lat); return x,y
    def m_to_ll(self, x, y):
        lon,lat = self.to_deg.transform(x, y); return lat,lon
...
class DronePathPlanner:
        ...
        for _, b in self.buildings.iterrows():
            ...
            try:
                if g.geom_type == 'Polygon':
                    ...
                    bufs.append(None); continue
                bufs.append(poly.buffer(self.avoid_distance))
            except Exception:
                bufs.append(None)
        self.buildings['buffer_xy'] = bufs
        valid = [b for b in bufs if b is not None]
    ...
    def check_collision(self, lat: float, lon: float, alt: float) -> Tuple[bool, float]:
        ...
        # --- PATCH: metric collision via precomputed buffers ---
        x,y = self._proj.ll_to_m(lat, lon)
        p_xy = Point(lon, lat)  # keep original for compatibility
        p_xy_m = Point(x, y)
        if getattr(self, "_union_buffer", None) is not None and not self._union_buffer.contains(p_xy_m):
            return False, 0.0
        for idx, building in self.buildings.iterrows():
            buf = building.get('buffer_xy', None)
            if buf is None: continue
            if buf.contains(p_xy_m):
                if alt < building['height_m'] + self.avoid_distance:
                    return True, building['height_m']
    ...
    def _plan_horizontal_path(self,
        ...
        while iteration < max_iterations:
            ...
            scale = self.step_size / 111000
            step_lat = current_lat + dlat * scale
            step_lon = current_lon + dlon * scale
