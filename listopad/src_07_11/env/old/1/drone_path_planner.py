...
class DronePathPlanner:
    ...
    def check_collision(self, lat: float, lon: float, alt: float) -> bool:
        ...
        point = Point(lon, lat)
        buffer_deg = self.config.avoid_distance / 111000  # ~111km per degree
        point_buffered = point.buffer(buffer_deg)

        for idx, building in self.buildings.iterrows():
            # Sprawdź czy punkt (z buforem) przecina budynek
            if building.geometry.intersects(point_buffered):
                # Sprawdź wysokość
    ...
    def find_safe_altitude(self, lat: float, lon: float) -> float:
        ...
        point = Point(lon, lat)
        buffer_deg = self.config.avoid_distance / 111000
        point_buffered = point.buffer(buffer_deg)
        ...
        for idx, building in self.buildings.iterrows():
            if building.geometry.intersects(point_buffered):
                required_height = building['height_m'] + self.config.avoid_distance
    ...
    def _plan_horizontal_path(self,
        ...
        num_steps = int(distance_2d * 111000 / self.config.step_size)  # ~111km per degree
