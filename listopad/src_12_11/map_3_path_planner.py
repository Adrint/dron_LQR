...
class DroneRoutePlanner:
    ...
    def _plot_overview_and_profile(self, path_latlon, latA, lonA, latB, lonB, buildings):
        ...
        # --- FIGURA I OSIE: stylistyka jak w map_2 ---
        fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(14, 10))
        ax1.set_aspect("equal", adjustable="datalim")
        ax1.set_facecolor("#f9f9f9")
        ax2.set_facecolor("#f9f9f9")

        # --- Budynki w widoku z góry: te same kolory/krawędzie jak w map_2 ---
        if b_in is not None and len(b_in) > 0:
        ...
        # --- Linia prosta A–B (jak w map_2) + trasa planera ---
        ...
        # --- DRUGI WYKRES: zabudowa poniżej aktualnej wysokości lotu ---
        # tniemy zabudowę do profilu wysokości
        clipped_h = np.minimum(bldg_h, alts_arr) if alts_arr.size else bldg_h.copy()

        # wypełnienie w kolorze budynków jak w map_2
        ax2.fill_between(
            s, 0.0, bldg_h,
            facecolor=COLORS["building"],  # ten sam pomarańczowy co w map_2
            alpha=0.6,
            label="Wysokość zabudowy",
            zorder=20
        )
        # cienki obrys jak w map_2
        ax2.plot(s, bldg_h, linewidth=0.5, color="darkgray", zorder=21)

        # profil wysokości zaplanowanej trasy
...
def compute_building_profile_along_path(x_m, y_m, buildings_gdf, ref_lat, ref_lon, angle_rad, latlon_to_meters, estimate_height):
    ...
    # tolerancja ~ 1.5 m w stopniach
    tol_deg = 1.5 / 111000.0
    ...
    for i, (xx, yy) in enumerate(zip(x_m, y_m)):
        ...
        # metry -> geo (przez prostą iterację lokalną)
        # tu wykorzystaj swoją istniejącą odwrotną projekcję, albo mały offset
        # najprościej: przeszukaj w okienku (cx) wokół ostatniej znanej (lat,lon)
        around = buildings_gdf  # jeśli masz .cx użyj jak w _plot_overview_and_profile
        hmax = 0.0
        p = Point(0, 0)  # placeholder; w swoim kodzie użyj realnych (lon,lat)
        for _, b in around.iterrows():
            try:
                geom = b.geometry
                if geom is None:
...
# W map_3_path_planner.py podczas przygotowania rysunku/generacji trasy:
def export_buildings_xy(buildings_gdf, ref_lat, ref_lon, angle_rad, latlon_to_meters):
    ...
    for _, b in buildings_gdf.iterrows():
        ...
        for g in geoms:
            ...
            # obrót o angle_rad (tak jak trasa)
