...
class DroneVisualizer:
    ...
    def __init__(self, X_ref_all, Y_terr_all, Z_terr_all, Y_ref_all, Z_ref_all):
        ...
        # Wczytaj budynki dla wizualizacji
        self.buildings = self._load_buildings()

        # Przygotuj dane dla wizualizacji jak w map_3_path_planner
    ...
    def _load_buildings(self):
        """Wczytaj budynki z pliku dla wizualizacji"""
        from pathlib import Path
        import pandas as pd

        buildings_path = Path("data/warsaw_buildings.pkl")
        if buildings_path.exists():
            try:
                buildings = pd.read_pickle(buildings_path)
                print(f"[Visualization] Wczytano {len(buildings)} budynków dla wizualizacji")
                return buildings
            except Exception as e:
                print(f"[Visualization] ⚠️ Błąd wczytywania budynków: {e}")
                return None
        else:
            print(f"[Visualization] ⚠️ Brak pliku z budynkami")
            return None

    @staticmethod
    def _rotate_points(x, y, angle_rad):
        """Rotacja punktów o kąt angle_rad"""
    ...
    def _prepare_visualization_data(self):
        """Przygotuj dane do wizualizacji w stylu map_3_path_planner"""
        # Oblicz kąt rotacji aby trasa była pozioma (A->B)
        xa, ya = self.X_ref_all[0], self.Y_ref_all[0]
        xb, yb = self.X_ref_all[-1], self.Y_ref_all[-1]
        self.angle = -np.arctan2((yb - ya), (xb - xa))

        # Rotuj współrzędne trasy
        self.px_r, self.py_r = self._rotate_points(self.X_ref_all, self.Y_ref_all, self.angle)

        # Oblicz granice dla ax1 (widok z góry)
        xa_r, ya_r = self._rotate_points(np.array([xa]), np.array([ya]), self.angle)
        xb_r, yb_r = self._rotate_points(np.array([xb]), np.array([yb]), self.angle)
        xa_r, ya_r = xa_r[0], ya_r[0]
        xb_r, yb_r = xb_r[0], yb_r[0]

        self.x_min_map = min(self.px_r.min(), xa_r, xb_r) - 20
        self.x_max_map = max(self.px_r.max(), xa_r, xb_r) + 20
        y_center = 0.5 * (ya_r + yb_r)
        y_span = max(100.0, min(300.0, max(abs(self.py_r - y_center)) + 50.0))
        self.y_min_map = y_center - y_span
        self.y_max_map = y_center + y_span

        # Oblicz dystans wzdłuż trasy dla ax2
        dx = np.diff(self.X_ref_all)
        dy = np.diff(self.Y_ref_all)
        ds = np.hypot(dx, dy)
        self.s = np.concatenate([[0.0], np.cumsum(ds)])

        # Oblicz wysokości budynków wzdłuż trasy
        self.bldg_h = self._calculate_building_heights_along_path()

        print(f"[Visualization] Przygotowano dane:")
        print(f"  Dystans trasy: {self.s[-1]:.1f} m")
        print(f"  Max wysokość budynków: {self.bldg_h.max():.1f} m")

    def _calculate_building_heights_along_path(self):
        """Oblicz wysokości budynków wzdłuż ścieżki"""
        bldg_h = np.zeros_like(self.s)

        if self.buildings is None or len(self.buildings) == 0:
            return bldg_h

        # To jest uproszczona wersja - dla pełnej implementacji potrzebna
        # byłaby konwersja geo<->metry z punktem referencyjnym
        # Na razie używamy maksymalnej wysokości w regionie
        try:
            max_height = 0.0
            for _, bldg in self.buildings.iterrows():
                try:
                    h = self._estimate_height(bldg)
                    if h > max_height:
                        max_height = h
                except Exception:
                    continue

            # Symuluj profil zabudowy - większa w środku trasy
            mid_idx = len(self.s) // 2
            for i in range(len(self.s)):
                # Funkcja trójkątna - max w środku
                distance_from_mid = abs(i - mid_idx) / mid_idx
                bldg_h[i] = max_height * 0.5 * (1.0 - distance_from_mid * 0.5)

        except Exception as e:
            print(f"[Visualization] ⚠️ Błąd obliczania wysokości budynków: {e}")
    ...
    def _init_plots(self):
        ...
        # ===================================================================
        # Subplot 1: Widok z góry Y(X) z budynkami - jak w map_3_path_planner
        # ===================================================================
        
        # ===================================================================
        # Subplot 3: 3D Trajectory - TYLKO reference i dron (bez terenu)
        # ===================================================================
        self.ax1 = plt.subplot(1, 2, 1, projection='3d')
        self.ax1.plot(self.X_ref_all, self.Y_ref_all, self.Z_ref_all, 'r-',
                     label='Reference', linewidth=2)
        self.ax1.set_xlabel('X [m]')
        self.ax1.set_ylabel('Y [m]')
        self.ax1.set_zlabel('Z [m] (NED)')
        self.ax1.set_title('Trajektoria 3D (X, Y, Z)', fontsize=10)
        self.ax1.legend(fontsize=10)
        self.ax1.grid(True, alpha=0.3)
        self.ax1.invert_zaxis()

        # ===================================================================
        # Subplot 4: Drone Orientation (bottom right) - 3D VIEW - BEZ ZMIAN
        # ===================================================================
    ...
    def update_plots(self, x, t, i, yp_array, T1, T2, T3, T4, limits_exceeded):
        ...
        # Negacja theta bo aa_mdl używa matematycznej konwencji rotacji (CCW dla theta > 0)
        # a NED aerospace ma theta > 0 = pitch up
        xs, zs = aa_mdl(X_pos, Z_pos, -theta, 5)
        xsy, ys = aa_mdl(X_pos, Y_pos, -phi, 5)
        alt = -Z_pos
        # ====================================================================
        # UPDATE SUBPLOT 1: WIDOK Z GÓRY Y(X) Z BUDYNKAMI
        # ====================================================================
        
        # ====================================================================
        # UPDATE SUBPLOT 3: 3D TRAJECTORY - TYLKO reference i dron
        # ====================================================================
        self.ax1.clear()
        self.ax1.plot(self.X_ref_all, self.Y_ref_all, self.Z_ref_all, 'r-',
                     label='Reference', linewidth=2)
        ...
        self.ax1.scatter([yp_array[i, 6]], [yp_array[i, 7]], [yp_array[i, 8]],
                        color='blue', s=100)
    ...
    def _draw_drone_orientation(self, x, T1, T2, T3, T4, phi_deg, theta_deg, psi_deg):
        ...
        #self.ax2.view_init(elev=20, azim=135)
        ...
        # Use half-dimensions because vertices go from -r to +r
