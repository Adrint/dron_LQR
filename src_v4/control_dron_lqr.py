...
def dron_lqr(path_result=None):
    ...
    z_absolute_ground = 0.0  #Limit dla drona, aby nie mógł lecieć poniżej ziemi

    protection_config = {
        'z_start': z_start,
        'z_initial_protection': z_start,
        'initial_protection_active': (z_start < -0.1)  # Aktywne jeśli start nad ziemią
    }

    print(f"Wysokość startowa: Z = {z_start:.2f}m (NED) = {-z_start:.2f}m (Z+)")
    ...
    # Inicjalizacja z początkową pozycją
    animator.hist_x.append(x_start)
    animator.hist_y.append(y_start)
    animator.hist_z.append(-z_start)  # Konwersja NED -> Z+
    ...
    for i in range(max_iterations):
        ...
        # Linearyzacja i LQR
        A, B = aa_matrices_AB(x, t, u, az_turbulence)
        K, P = lqr_m(A, B, Q, R)
        ...
        x = aa_rk45(x, t, dt, u, az_turbulence)
