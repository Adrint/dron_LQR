...
# ============================================================================
# CONFIGURATION - QUADCOPTER X CONFIGURATION (3D ONLY)
# ============================================================================
...
if z_start_input.strip() == "":
    z_start_AGL = 1.0  # Altitude Above Ground Level
else:
...
# ============================================================================
# HELPER FUNCTION: DRAW ANGLE ARC IN 3D WITH PROPER SIGN HANDLING
# ============================================================================
...
T_baseline = MASS * g / 4.0  # Thrust baseline for equilibrium
...
for i in range(10000):
    ...
    # Transform to body frame velocities (simplified)
    vx_ref = Vx_ref
    vy_ref = Vy_ref
    vz_ref = Vz_ref
    ...
    # Get full rotation matrix using all three Euler angles
    ...
    if abs(phi_deg) > 2:  # Only show if significant
    ...
    if abs(theta_deg) > 2:  # Only show if significant
        draw_angle_arc_3d(ax4, np.array([0, 0, 0]), 4,
                         np.array([1, 0, 0]), rotate_point([1, 0, 0], R),
                         np.array([0, 1, 0]), 'orange', f'θ={theta_deg:.1f}°')
    
    # Yaw angle (psi) - rotation around Z axis - largest radius
    if abs(psi_deg) > 2:  # Only show if significant
        draw_angle_arc_3d(ax4, np.array([0, 0, 0]), 2,
                         np.array([1, 0, 0]), rotate_point([1, 0, 0], R),
                         np.array([0, 0, 1]), 'cyan', f'ψ={psi_deg:.1f}°')

    # Info box with orientation data
