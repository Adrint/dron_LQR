...
def dron_lqr(path_result=None):
    ...
    drone_orientation = DroneVisualizer()

    # Inicjalizacja kontrolera symulacji
    controller = SimulationController()
    controller.start()
    time.sleep(0.5)  # Krótkie opóźnienie na uruchomienie GUI
    ...
    if path_result is not None:
        ...
        z_start = path_result['Z'][0]  # To jest już w NED
    else:
        x_start = x_ref_all[0]
        y_start = y_ref_all[0]
        z_start = z_ref_all[0]
    ...
    # 1. Absolute ground (Z=0) - drone can NEVER go below this
    # 2. Initial protection zone - for high-altitude starts, protects first 10m of descent
